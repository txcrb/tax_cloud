...
* Raise `TextCloud::Errors::MissingConfigError` when TaxCloud.configure has not been called - @dblock.
* Raise `TextCloud::Errors::MissingConfigOptionError` when TextCloud is not configured with an ID and API key - @dblock.
